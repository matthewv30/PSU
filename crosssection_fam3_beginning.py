...
radmin, radmax= 22.5,32.5
azimuth_lines = [136, 113]  # Two azimuth values
domain_width = 50
...
for Xsec_azimuth in range(start_azimuth, end_azimuth + 1, azimuth_step):
    ...
    fig, axs = plt.subplots(3, 2, figsize=(6.5*1.25, 10*1.25))
    ...
    Wcrosssection2, Xsec_dists2 = calculate_cross_section(LLctr, index_center, wwind, 113)
    iscrosssection, Xsec_dists = calculate_cross_section(LLctr, index_center, radial_wind, Xsec_azimuth)
    iscrosssection2, Xsec_dists2 = calculate_cross_section(LLctr, index_center,radial_wind, 113)
    W_cbrange = np.arange(-10,10.0001,1)
    vrad_cbrange = np.arange(-20,20.0001,1)
    tan_range=np.arange(-15,15.1,1)
    ...
    axs[4].set_ylim(5, 18)
    axs[5].set_ylim(5, 18)
    axs[1].set_xlim(90, 181)
    axs[1].set_ylim(5, 18)
    axs[1].set_xticks(np.arange(90,181,20))
    ...
    cbar.ax.set_title('m/s')
    axs[4].set_title(f"Radial Cross Section-\nVertical Velocity at {113} Degrees", fontsize=10)
    ...
    Wcfill3 = axs[3].contourf(Xsec_dists, zlevels[:] / 1000, iscrosssection, 
                            cmap=cm.seismic, levels=np.arange(-20,20.01,1), extend='both')
...
for azimuth_line in azimuth_lines:
    ...
    file_name = '/rita/s0/mjv5638/plots/cross_sections/26_16_5_0/v3/TEST_divergence_cross_section_{:03d}_trajectory_{}_{}_{}_{}_r{}_{}.png'.format(
