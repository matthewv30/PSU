...
radmin, radmax= 15,35
#radmin_radavg, radmax_radavg= 25,30 #Used in radial average 
...
for idx, modellevel in enumerate(range(2, 36 * 2, 2 * 2)):
    ...
    #Supersaturation
    supersaturation_copy = np.copy(supersaturation_ice)  # Replace wwind with supersaturation
    ew_mean_ud, altitude, radiusmin, radiusmax = radial_mean(LLctr, supersaturation_copy[modellevel, :, :], modellevel)
...
for Xsec_azimuth in range(start_azimuth, end_azimuth + 1, azimuth_step):
    ...
    Wcrosssection2, Xsec_dists2 = calculate_cross_section(LLctr, index_center, wwind, 175)
    iscrosssection, Xsec_dists = calculate_cross_section(LLctr, index_center, supersaturation_ice, Xsec_azimuth)
    iscrosssection2, Xsec_dists2 = calculate_cross_section(LLctr, index_center, supersaturation_ice, 175)
    ...
    for idx, color, label in zip(trajectory_indices, colors, legend_labels):
        ...
        ax = axs[2]  
        ax.plot(d, ictg['part_posz'][i, j, k, s, :], linewidth=1.5, color=color, label=label)
        ax.plot(d[0], ictg['part_posz'][i, j, k, s, 0], marker="o", markersize=4, color=color)

        # Plot the trajectory for the azimuth plot
        axs[1].plot(azimuths, ictg['part_posz'][i, j, k, s, :], linewidth=1.5, color=color, label=label)
        axs[1].plot(azimuths[0], ictg['part_posz'][i, j, k, s, 0], marker="o", markersize=4, color='black')

        axs[2].plot(azimuths, ictg['part_posz'][i, j, k, s, :], linewidth=1.5, color=color, label=label)
        axs[2].plot(azimuths[0], ictg['part_posz'][i, j, k, s, 0], marker="o", markersize=4, color='black')

        axs[3].plot(d, ictg['part_posz'][i, j, k, s, :], linewidth=1.5, color=color, label=label)
        axs[4].plot(d, ictg['part_posz'][i, j, k, s, :], linewidth=1.5, color=color, label=label)
        axs[5].plot(d, ictg['part_posz'][i, j, k, s, :], linewidth=1.5, color=color, label=label)

        #Put a marker at specified time
    ...
    for idx, color, label in zip(trajectory_indices, colors, legend_labels):
        ...
        axs[0].plot(x[0], y[0], marker="o", markersize=4, color='black')
        axs[0].plot(x[time1], y[time1], marker="X", markersize=6, color=color)
        axs[0].plot(x[time2], y[time2], marker="X", markersize=6, color=color)

    axs[1].set_title(f"Azimuthal Cross Section- Vertical Velocity", fontsize=10)
    axs[1].set_xlabel('Azimuth (degrees)')
    axs[1].set_ylabel('Altitude (km)')
    axs[1].text(0.02, 0.92, "b)", transform=axs[1].transAxes, fontsize=18)

    axs[1].set_xlim(90, 226)
    axs[1].set_ylim(5, 18)
    axs[1].set_xticks(np.arange(90,226,45))
    ...
    w= axs[1].contourf(azimuthalbins[keep_w], altkmlist_w, allew_ud_w, levels = np.arange(-5,5.1,0.1), cmap=cm.seismic, extend='both') #Vertical Velocity
    cbar = plt.colorbar(w, ax=axs[1])
    cbar.set_ticks(np.arange(-5, 5.01, 1)) 
    cbar.ax.set_title('m/s')
    
    # Vertical Velocity Cross Section 1
    Wcfill = axs[2].contourf(Xsec_dists, zlevels[:] / 1000, Wcrosssection, 
                            cmap=cm.seismic, levels=W_cbrange, extend='both', alpha=0.75)
    cbar = plt.colorbar(Wcfill, ax=axs[2])
    cbar.ax.set_title('m/s')
    axs[2].set_title(f"Radial Cross Section-\nVertical Velocity at {Xsec_azimuth} Degrees", fontsize=10)
    ...
    Wcfill2 = axs[4].contourf(Xsec_dists2, zlevels[:] / 1000, Wcrosssection2, 
                            cmap=cm.seismic, levels=W_cbrange, extend='both', alpha=0.75)
    cbar = plt.colorbar(Wcfill2, ax=axs[4])
    cbar.ax.set_title('m/s')
    axs[4].set_title(f"Radial Cross Section-\nVertical Velocity at {175} Degrees", fontsize=10)
    axs[4].set_xlabel('Radial distance from center (km)')
    axs[4].set_ylabel('Altitude (km)')
    axs[4].text(0.02, 0.92, "e)", transform=axs[4].transAxes, fontsize=18)

    Wcfill3 = axs[3].contourf(Xsec_dists, zlevels[:] / 1000, iscrosssection, 
                            cmap=cm.BrBG, levels=np.arange(-0.3,0.31,0.05), extend='both')
    cbar = plt.colorbar(Wcfill3, ax=axs[3])

    axs[3].set_title(f"Radial Cross Section-\nIce Supersaturation at {Xsec_azimuth} Degrees", fontsize=10)
    axs[3].set_xlabel('Radial distance from center (km)')
    axs[3].set_ylabel('Altitude (km)')
    axs[3].text(0.02, 0.92, "d)", transform=axs[3].transAxes, fontsize=18, color='white')

    Wcfill5 = axs[5].contourf(Xsec_dists2, zlevels[:] / 1000, iscrosssection2, 
                            cmap=cm.BrBG, levels=np.arange(-0.3,0.31,0.05), extend='both')
    cbar = plt.colorbar(Wcfill5, ax=axs[5])
    
    axs[5].set_title(f"Radial Cross Section-\nIce Supersaturation at {175} Degrees", fontsize=10)
    axs[5].set_xlabel('Radial distance from center (km)')
    axs[5].set_ylabel('Altitude (km)')
    axs[5].text(0.02, 0.92, "f)", transform=axs[5].transAxes, fontsize=18, color='white')
...
axs[4].set_xlim(radmin, radmax)
axs[5].set_xlim(radmin, radmax)

azimuth_lines = [175, 200]  # Two azimuth value
radius = 150  # Can change

for azimuth_line in azimuth_lines:
    # Plot vertical dashed lines on the azimuth plots
    axs[1].axvline(x=azimuth_line, color='black', linestyle='--', linewidth=1)
    ...
    file_name = '/rita/s0/mjv5638/plots/A_thesis/TEST_cross_section_{:03d}_trajectory_{}_{}_{}_{}_r{}_{}.png'.format(
