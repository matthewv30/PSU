...
azimuth_step = 1
trajectory_indices = [(26,16,5,1)] #Unique and Extreme Family
radmin, radmax= 25, 35 #Range
...
for Xsec_azimuth in range(start_azimuth, end_azimuth + 1, azimuth_step):
    ...
    # Create axs[2] and axs[3] first to get their positions
    axs2 = fig.add_subplot(gs[1, 0])
    axs3 = fig.add_subplot(gs[1, 1])

    # Get their positions
    pos2 = axs2.get_position()
    pos3 = axs3.get_position()

    # Compute center x and width
    new_x0 = (pos2.x0 + pos3.x0) / 2
    new_width = pos2.width  # same width as other plots

    # Create axs[0] manually using fig.add_axes()
    axs0 = fig.add_axes([new_x0, pos2.y0 + (pos2.height + 0.055), new_width, pos2.height])

    # axs[1]: placeholder
    axs1 = None

    # Rebuild axs[2] and axs[3] using the same gs
    axs_rest = [axs2, axs3]

    # Create axs[4] to axs[7] in remaining rows
    for i in range(4):
        row = 2 + i // 2  # rows 2 and 3
        col = i % 2
        axs_rest.append(fig.add_subplot(gs[row, col]))

    axs = [axs0, axs1] + axs_rest

    # List of specific trajectory indices
    colors = ['darkred'] #Unique and Extreme Family
    legend_labels = ['FC5'] 
    
    #Xsec_azimuth = azimuth_step  
    rwcrosssection_1, Xsec_dists_1 = calculate_cross_section(LLctr, index_center, radial_wind, start_azimuth)
    rwcrosssection_2, Xsec_dists_2 = calculate_cross_section(LLctr, index_center, radial_wind, azimuth_2)
    rwcrosssection_3, Xsec_dists_3 = calculate_cross_section(LLctr, index_center, radial_wind, azimuth_3)
    
    iscrosssection_1, Xsec_dists_1 = calculate_cross_section(LLctr, index_center, supersaturation_ice, start_azimuth)
    iscrosssection_2, Xsec_dists_2 = calculate_cross_section(LLctr, index_center, supersaturation_ice, azimuth_2)
    iscrosssection_3, Xsec_dists_3 = calculate_cross_section(LLctr, index_center, supersaturation_ice, azimuth_3)
    ...
    axs[0].text(0.02, 0.88, "a)", transform=axs[0].transAxes, fontsize=18)
    ...
    for idx, color, label in zip(trajectory_indices, colors, legend_labels):
        ...
        axs[4].plot(d[time2], ictg['part_posz'][i, j, k, s, time2], marker="X", markersize=10, markeredgecolor='white', markeredgewidth=1, color=color)
        axs[5].plot(d[time2], ictg['part_posz'][i, j, k, s, time2], marker="X", markersize=10, markeredgecolor='white', markeredgewidth=1, color=color)
        axs[6].plot(d[time3], ictg['part_posz'][i, j, k, s, time3], marker="X", markersize=10, markeredgecolor='white', markeredgewidth=1, color=color)
        axs[7].plot(d[time3], ictg['part_posz'][i, j, k, s, time3], marker="X", markersize=10, markeredgecolor='white', markeredgewidth=1, color=color)

    axs[2].set_title(f"Radial Cross Section- Ice Supersaturation at {Xsec_azimuth} Degrees", fontsize=10)
    axs[2].set_xlabel('Radial distance from center (km)')
    axs[2].set_ylabel('Altitude (km)')
    axs[2].text(0.02, 0.88, "b)", transform=axs[2].transAxes, fontsize=18, color= 'white')

    axs[3].set_title(f"Radial Cross Section- Radial Velocity \nand Vertical Velocity at {Xsec_azimuth} Degrees", fontsize=10)
    axs[3].set_xlabel('Radial distance from center (km)')
    axs[3].set_ylabel('Altitude (km)')
    axs[3].text(0.02, 0.88, "c)", transform=axs[3].transAxes, fontsize=18)

    axs[4].set_title(f"Radial Cross Section- Ice Supersaturation at {azimuth_2} Degrees", fontsize=10)
    axs[4].set_xlabel('Radial distance from center (km)')
    axs[4].set_ylabel('Altitude (km)')
    axs[4].text(0.02, 0.88, "d)", transform=axs[4].transAxes, fontsize=18, color= 'white')

    axs[5].set_title(f"Radial Cross Section- Radial Velocity \nand Vertical Velocity at {azimuth_2} Degrees", fontsize=10)
    axs[5].set_xlabel('Radial distance from center (km)')
    axs[5].set_ylabel('Altitude (km)')
    axs[5].text(0.02, 0.88, "e)", transform=axs[5].transAxes, fontsize=18)

    axs[6].set_title(f"Radial Cross Section- Ice Supersaturation at {azimuth_3} Degrees", fontsize=10)
    axs[6].set_xlabel('Radial distance from center (km)')
    axs[6].set_ylabel('Altitude (km)')
    axs[6].text(0.02, 0.88, "f)", transform=axs[6].transAxes, fontsize=18, color= 'white')

    axs[7].set_title(f"Radial Cross Section- Radial Velocity \nand Vertical Velocity at {azimuth_3} Degrees", fontsize=10)
    axs[7].set_xlabel('Radial distance from center (km)')
    axs[7].set_ylabel('Altitude (km)')
    axs[7].text(0.02, 0.88, "g)", transform=axs[7].transAxes, fontsize=18)
   
    # Vertical Velocity Cross Section 1
    Wcfill = axs[3].contourf(Xsec_dists_1, zlevels[:] / 1000, rwcrosssection_1, 
                            cmap=cm.seismic, levels=rv_range, extend='both', alpha=0.9)
    cbar = plt.colorbar(Wcfill, ax=axs[3])
    cbar.ax.set_title('m/s')
    tangential=axs[3].contour(Xsec_dists_1, zlevels[:] / 1000, Wcrosssection_1, linewidths=0.7,
                            levels = W_cbrange, colors='black', extend='both')
    # Define y-locations where you want to label contours at x=30
    y_positions = np.linspace(np.min(zlevels) / 1000, np.max(zlevels) / 1000, 8)
    manual_positions = [(15, y) for y in y_positions]  # Force labels at x=30
    ...
    # Add labels manually at those positions
    Wcfill = axs[2].contourf(Xsec_dists_1, zlevels[:] / 1000, iscrosssection_1, 
                            cmap=cm.BrBG, levels=np.arange(-0.3,0.31,0.05), extend='both', alpha=0.9)
    cbar = plt.colorbar(Wcfill, ax=axs[2])
 

     # Vertical Velocity Cross Section 2
    Wcfill = axs[5].contourf(Xsec_dists_2, zlevels[:] / 1000, rwcrosssection_2, 
                            cmap=cm.seismic, levels=rv_range, extend='both', alpha=0.9)
    cbar = plt.colorbar(Wcfill, ax=axs[5])
    cbar.ax.set_title('m/s')

    tangential=axs[5].contour(Xsec_dists_2, zlevels[:] / 1000, Wcrosssection_2, linewidths=0.7,
                            levels = W_cbrange, colors='black', extend='both')
    # Define y-locations where you want to label contours at x=30
    y_positions = np.linspace(np.min(zlevels) / 1000, np.max(zlevels) / 1000, 10)
    manual_positions = [(10, y) for y in y_positions]  # Force labels at x=30
    ...
    Wcfill = axs[7].contourf(Xsec_dists_3, zlevels[:] / 1000, rwcrosssection_3, 
                            cmap=cm.seismic, levels=rv_range, extend='both', alpha=0.9)
    cbar = plt.colorbar(Wcfill, ax=axs[7])
    cbar.ax.set_title('m/s')
    tangential=axs[7].contour(Xsec_dists_3, zlevels[:] / 1000, Wcrosssection_3, linewidths=0.7,
                            levels = W_cbrange, colors='black', extend='both')
    # Define y-locations where you want to label contours at x=30
    y_positions = np.linspace(np.min(zlevels) / 1000, np.max(zlevels) / 1000, 10)
    manual_positions = [(5, y) for y in y_positions]  # Force labels at x=30
    ...
    #Ice Supersaturation 2
    Wcfill = axs[4].contourf(Xsec_dists_2, zlevels[:] / 1000, iscrosssection_2, 
                            cmap=cm.BrBG, levels=np.arange(-0.3,0.31,0.05), extend='both', alpha=0.9)
    cbar = plt.colorbar(Wcfill, ax=axs[4])

     #Ice Supersaturation 3
    Wcfill = axs[6].contourf(Xsec_dists_3, zlevels[:] / 1000, iscrosssection_3, 
                            cmap=cm.BrBG, levels=np.arange(-0.3,0.31,0.05), extend='both', alpha=0.9)
    cbar = plt.colorbar(Wcfill, ax=axs[6])
    ...
    file_name = '/rita/s0/mjv5638/plots/cross_sections/26_16_5_0/v3/TEST_sb2_cross_section_{:03d}_trajectory_{}_{}_{}_{}.png'.format(
